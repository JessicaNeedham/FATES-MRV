...
def appf_to_ap_by_pft(appf_var, dataset):
    """function to reshape a fates multiplexed patch age and pft-indexed variable to one indexed by patch age and pft
    first argument should be an xarray DataArray that has the FATES APPF dimension
    second argument should be an xarray Dataset that has the FATES PFT dimension 
    (possibly the dataset encompassing the dataarray being transformed)
    returns an Xarray DataArray with the patch age and pft dimensions disentangled"""
    n_age = len(dataset.fates_levage)
    ds_out = (appf_var.rolling(fates_levagepft=n_age, center=False)
            .construct("fates_levage")
            .isel(fates_levagepft=slice(n_age-1, None, n_age))
            .rename({'fates_levagepft':'fates_levpft'})
            .assign_coords({'fates_levage':dataset.fates_levage})
            .assign_coords({'fates_levpft':dataset.fates_levpft}))
    ds_out.attrs['long_name'] = appf_var.attrs['long_name']
    ds_out.attrs['units'] = appf_var.attrs['units']
    return(ds_out)
